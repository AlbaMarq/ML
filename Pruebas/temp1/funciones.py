...
def transformar(df, scaler):
	# Cargar valores (solo etiquetas)
	values = df['temperatura'].values
	# Pasar a tipo float
	values = values.astype('float32')
	# Normalizar características (ponerlas en vertical)
	values = values.reshape(-1, 1)
	# Escalar los valores
	scaled = scaler.fit_transform(values)
	print(scaled)
	
    # Remuestreo de valores (validación cruzada????)
	reframed = series_to_supervised(scaled, PASOS, 1)
	reframed.reset_index(inplace=True, drop=True)
	
    # Ponemos los días y meses primeros en la tabla
	contador = 0
	reframed['dia'] = df['dia']
	reframed['mes'] = df['mes']
...
'''' Convertir las series para aprendizaje supervisado '''
